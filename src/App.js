...
import NewContentIndicator from './components/NewContentIndicator';
...
class App extends Component {
  ...
  componentDidMount = () => {
    ...
    Analytics.set("/");
    Analytics.pageview("/");
    window.showNewContentIndicator = this.showNewContentIndicator;
  };
  runningWorker = null;
  getStateFromHash = () => {
    ...
    this.runningWorker = decode(this.props.location.hash.slice(1))
      .then(value => {
        this.runningWorker = null;
        const validShape =
          value.hasOwnProperty("cards") && value.hasOwnProperty("title");
        value = {
          ...value,
  ...
  render() {
    return (
      <div className="App">
        ...
        {this.state.showNewContentIndicator && (
          <NewContentIndicator
            onRequestClose={this.hideNewContentIndicator}
          />
        )}
        <Route
          render={({ location }) => (
